...
function M.sound()
	-- linux
	-- os.execute("timeout 1 aplay " .. os.getenv("HOME") .. "/.local/share/nvim/lazy/countdown/media/pr_in_space.mp3")
	os.execute("timeout 1 aplay ../../media/pr_in_space.mp3")
end
...
function M.countdown(minus)
	...
	countdown_job_id = vim.fn.jobstart({
		...
		"-c",
		string.format(
			[[for i in $(seq %d -1 1); do echo "You set work duration for %d minutes \nCountdown: $i seconds remaining"; sleep 1; done; echo 'Countdown: Time is up!']],
			duration,
			minus
		),
	}, {
